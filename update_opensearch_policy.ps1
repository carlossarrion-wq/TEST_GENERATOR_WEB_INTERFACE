$policy = @"
{
  "Version": "2012-10-17",
  "Statement": [
    {
      "Effect": "Allow",
      "Principal": {
        "AWS": [
          "arn:aws:iam::701055077130:role/rag-system-production-RAGEC2Role-hawdzi5Lrv3d",
          "arn:aws:iam::701055077130:role/TestPlanGeneratorLambdaRole"
        ]
      },
      "Action": "es:*",
      "Resource": "arn:aws:es:eu-west-1:701055077130:domain/rag-opensearch-clean/*"
    }
  ]
}
"@

$policyMinified = $policy -replace '\s+', ' ' -replace '> <', '><'

aws opensearch update-domain-config --domain-name rag-opensearch-clean --region eu-west-1 --access-policies $policyMinified
