...
function setupPageRefreshProtection() {
    window.addEventListener('beforeunload', function(e) {
        ...
        if (testCases && testCases.length > 0) {
            // Modern browsers require both preventDefault() and returnValue to be set
            // The custom message is ignored by modern browsers for security reasons
            // They will show their own generic message like "Changes you made may not be saved"
            e.preventDefault();
...
function displayTestCases() {
    ...
    if (!emptyStateContainer) {
        ...
        if (tableContainer && tableContainer.parentNode) {
            tableContainer.parentNode.insertBefore(emptyStateContainer, tableContainer.nextSibling);
        }
    }
...
async function sendChatMessage() {
    ...
    if (!testCases || testCases.length === 0) {
        showWarningBlockingNotification(
            'Generar plan de pruebas primero',
            'Debes generar un plan de pruebas antes de poder usar el chat para refinar los casos de prueba.'
        );
        return;
    }
...
function saveTestCaseEdit(testCaseId) {
    ...
    showSuccessNotification(
        ...
        `${testCase.id} se ha modificado correctamente`
    );
}
...
async function deleteTestCase(testCaseId) {
    ...
    const userAccepted = await showWarningNotification(
        ...
        `¿Estás seguro de que quieres eliminar ${testCase.id}?\n\nEsta acción no se puede deshacer.`
    );
...
async function deleteAllTestCases() {
    ...
    // Show warning notification and wait for user response
    const userAccepted = await showWarningNotification(
        'Eliminar todos los casos de prueba',
        `¿Estás seguro de que quieres eliminar TODOS los ${numCases} casos de prueba?\n\nEsta acción no se puede deshacer.`
    );
